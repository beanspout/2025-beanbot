...
func NewClient(baseURL, model string) *Client {
	return &Client{
		...
		model:   model,
		client: &http.Client{
			Timeout: 60 * time.Second, // Increased timeout for model response generation
		},
	}
}
...
func (oc *Client) FindAvailableModel() (bool, string) {
	// Try common model names, prioritizing gemma3:1b
	models := []string{"gemma3:1b", "gemma2:1b", "gemma:1b", "llama3.2:1b", "llama2", "mistral", "qwen2.5:1.5b"}
...
func (oc *Client) GenerateResponse(prompt string) (string, error) {
	...
	if !oc.TestConnection() {
		log.Printf("Ollama not available, using fallback")
		return oc.generateFallbackResponse(prompt), nil
	}

	// Try to find an available model if the current one doesn't work
	if !oc.testModel(oc.model) {
		log.Printf("Model %s not available, searching for alternatives", oc.model)
		available, newModel := oc.FindAvailableModel()
		if !available {
			log.Printf("No models available, using fallback")
			return oc.generateFallbackResponse(prompt), nil
		}
		log.Printf("Switching to model: %s", newModel)
		oc.model = newModel
	}
	...
	if err != nil {
		log.Printf("Ollama request failed: %v, using fallback", err)
		return oc.generateFallbackResponse(prompt), nil
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		log.Printf("Ollama returned status %d, using fallback", resp.StatusCode)
		return oc.generateFallbackResponse(prompt), nil
	}

	var ollamaResp models.OllamaResponse
	if err := json.NewDecoder(resp.Body).Decode(&ollamaResp); err != nil {
		log.Printf("Failed to decode Ollama response: %v, using fallback", err)
		return oc.generateFallbackResponse(prompt), nil
	}
...
*Note: For full AI-powered troubleshooting, please install Ollama and run 'ollama pull gemma3:1b'*`, userIssue)
}
